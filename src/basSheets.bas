...
Private Function GetNamedRange(strNameIndex As String) As Range
...
HandleError:
    On Error GoTo 0
    MsgBox "Named Range Not Found" & " for '" + strNameIndex + "'", vbExclamation, "Barcode Generator::Named Range"
...
Dim sht As Worksheet
    If SheetExists(strSheetName, wkb) Then
        ...
        Else
            MsgBox "A sheet by the name " & strSheetName & " allready exists, new sheet not created", vbOKOnly, "Barcode Generator::Create Worksheet"
        End If
    End If
...
Dim cht As Chart
    ...
    If SheetExists(strSheetName) Then
        ...
        Else
            MsgBox "A sheet by the name " & strSheetName & " allready exists, new sheet not created", vbOKOnly, "Barcode Generator::Create Chart"
...
'' ToolResetSheetsCodeThisworkbook.NamesWill not work unless the Project is trusted in the trust center (Disabled option on Current Configuration)
'Private Sub ToolResetSheetsCodeGetNamedRange(strCodeNamePrefix As String)
...
Private Sub mSetWorkbookWindowVisibility(strWorkbookName As String, fVisible As Boolean)
    On Error Resume Next
    Application.Windows(strWorkbookName).Visible = fVisible
End Sub

Public Sub HideWorkbook(strWorkbookName As String)
    mSetWorkbookWindowVisibility strWorkbookName, False
End Sub

Public Sub RevealWorkbook(strWorkbookName As String)
    mSetWorkbookWindowVisibility strWorkbookName, True
End Sub

Public Sub HideSheet(sht As Worksheet)
    On Error Resume Next
    sht.Visible = xlSheetVeryHidden
    If sht.Visible <> xlSheetVeryHidden Then
        sht.Visible = XlSheetVisibility.xlSheetHidden
    End If
    On Error GoTo 0
End Sub

Public Sub RevealSheet(sht As Worksheet)
    On Error Resume Next
    sht.Visible = xlSheetVisible
    On Error GoTo 0
End Sub

Public Sub ToolRevealAllSheets()
    Dim sht As Worksheet
...
Public Sub ToolRevealAllWorkbooks()
    Dim wkb As Workbook
    For Each wkb In ThisWorkbook.Application.Workbooks
        RevealWorkbook wkb.Name
    Next wkb
    SetEcho True
End Sub
