...
Public Function FindUniqueCellRange( _
    ...
    wshCallingWorksheet As Worksheet, _
    Optional ByVal xlWholeOrxlPart As Long = xlPart) _
As Range

Dim c As Range
    Set c = wshCallingWorksheet.Cells.Find(strFindValue, , , xlWholeOrxlPart)
...
Public Function GetFirstRow(ByRef rngAddress As Range) As Long
Dim c As Range
Dim objArea As Variant
Dim lngLowestRow As Long
    For Each objArea In rngAddress.Areas
        For Each c In objArea.Rows
...
Public Function GetFirstColumn(ByRef rngAddress As Range) As Long
Dim c As Range
Dim objArea As Range
Dim lngLowestColumn As Long
    For Each objArea In rngAddress.Areas
        For Each c In objArea.Columns
...
Public Function GetLastColumn(ByRef rngAddress As Range) As Long
Dim c As Range
Dim objArea As Variant
...
Private Function GetAddressR1C1LastRow(ByRef rngAddress As Range, Optional ByRef intArea As Integer = 1) As Double
...
Dim rngFormulas As Range
    ...
    On Error Resume Next     'SpecialCells will fail when none are found
        Set rngConstants = rng.SpecialCells(xlCellTypeConstants, xlNumbers + xlTextValues)
        Set rngFormulas = rng.SpecialCells(xlCellTypeFormulas, xlNumbers + xlTextValues)
    ...
    Set GetNonBlankCellsFromRange = rngNonBlanks
